...
OPEN_ORDERS = {}  # Track open limit orders
...
def binance_signed_request(http_method, path, params=None):
    ...
    try:
        if http_method=="POST": return requests.post(url, headers=headers, params=params).json()
        elif http_method=="DELETE": return requests.delete(url, headers=headers, params=params).json()
        else: return requests.get(url, headers=headers, params=params).json()
...
def open_position(symbol, side, limit_price):
    ...
    if symbol in OPEN_ORDERS:
        try:
            binance_signed_request("DELETE", "/fapi/v1/order", {"symbol":symbol, "orderId":OPEN_ORDERS[symbol]})
            print(f"⚠️ Cancelled existing limit order for {symbol}")
        except: pass
...
def wait_and_notify_filled_entry(symbol, side, order_id):
    while True:
        ...
        if order_status.get("status")=="FILLED" and "avgFillPrice" in order_status:
...
def execute_market_exit(symbol, side):
    ...
    if "fills" in resp and len(resp["fills"])>0:
        avg_price = sum(float(f["price"])*float(f["qty"]) for f in resp["fills"]) / sum(float(f["qty"]) for f in resp["fills"])
        log_trade_exit(symbol,None,avg_price)
    else:
        # fallback using position entry price
        entry_price = float(pos_data[0]["entryPrice"])
        log_trade_exit(symbol,None,entry_price)
...
def async_exit_and_open(symbol, new_side, limit_price):
    def worker():
        ...
        opposite_side = None
        if amt>0 and new_side=="SELL": opposite_side="BUY"
        elif amt<0 and new_side=="BUY": opposite_side="SELL"
        if opposite_side:
