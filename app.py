...
from config import *
from trade_notifier import log_trade_entry, log_trade_exit  # Import notifier functions
...
def open_position(symbol, side, limit_price):
    ...
    # Notify Telegram only when order is filled
...
def execute_market_exit(symbol, side):
    ...
    # Notify Telegram immediately using executed price
    if "fills" in response and len(response["fills"])>0:
        avg_price = sum(float(f["price"])*float(f["qty"]) for f in response["fills"]) / sum(float(f["qty"]) for f in response["fills"])
